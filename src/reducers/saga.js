...
function* fetchQueue() {
    try {
        const response = yield call(fetch, `http://${window.location.host}:7000/api/queue`);
...
function* fetchHangmanGuesses() {
    try {
        const response = yield call(fetch, `http://${window.location.host}:7000/api/hangman`);
        const data = yield response.json();
        if (typeof data.messages !== 'undefined') {
            yield put({
                type: 'UPDATE_HANGMAN',
...
function* fetchShouldStopWatch() {
    try {
        const response = yield call(fetch, `http://${window.location.host}:7000/api/watch`);
        const data = yield response.json();
...
function* fetchSongQueue() {
    // check for new songs
    try {
        const response = yield call(fetch, `http://${window.location.host}:7000/api/song_requests`);
        const data = yield response.json();
        yield put({
    ...
    try {
        const response = yield call(fetch, `http://${window.location.host}:7000/api/should_skip_song`);
        const data = yield response.json();
    ...
    try {
        const response = yield call(fetch, `http://${window.location.host}:7000/api/should_pause_song`);
        const data = yield response.json();
...
function* saveTriviaQuestion(action) {
    try {
        ...
        const response = yield call(fetch, `http://${window.location.host}:7000/api/new_trivia_question', param`);
    } catch (e) {
        console.error(e);
    }
}
...
function* fetchTriviaQuestions() {
    try {
        const response = yield call(fetch, `http://${window.location.host}:7000/api/trivia_game`);
        const data = yield response.json();
...
function* fetchTriviaAnswers() {
    ...
    try {
        const response = yield call(fetch, `http://${window.location.host}:7000/api/trivia_answers`);
        const data = yield response.json();
...
function* fetchTriviaState() {
    ...
    try {
        const response = yield call(fetch, `http://${window.location.host}:7000/api/trivia_live`);
        const data = yield response.json();
        yield put({
...
function* fetchFriends() {
    try {
        const response = yield call(fetch, `http://${window.location.host}:7000/api/live_friends`);
        const data = yield response.json();
        yield put({
            type: 'UPDATE_FRIENDS',
